...
if [ -z $DFL_DIR/$projectName ]; then # If project title is empty
	...
	if [ -z $answer ] || ( [ $answer != "y" ] && [ $answer != "n" ] ); then # If libft choice is empty or invalid
		...
		cp -r templates/libft $DFL_DIR/$projectName/libs/libft # Add libft directory
	...
	cat >> $DFL_DIR/$projectName/Makefile < templates/Makefile.template # Add Makefile template
